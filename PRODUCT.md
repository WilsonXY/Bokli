...
- **Primary User (The Operator)**: Family food stall operator (primarily mom). Operates on an Android phone (Chrome browser) during evening wrap-up at the stall or at home after a long physical workday. Job is to log the day's revenue and costs in under 2 minutes, replace the handwritten paper book, and verify that the month reconciles without fatigue or arithmetic errors.
...
- **Operational Cadence**:
  ...
  - Month Close: Operator reviews the monthly summary, enters cash on hand and TnG balance on hand, inspects warn-only Reconciliation differences, and explicitly locks the month.
...
- **Operating Expenses**: Operating costs recorded per month (`rental`, `utilities`, `wages`, `other`).
- **Profit Calculation Engine**:
  - `Gross Profit` = Total Revenue (Cash + TnG) − Total Daily Costs
  - `Net Profit` = Gross Profit − Operating Expenses
- **Reconciliation**: Non-blocking check comparing (Cash on hand + TnG on hand) to Net Profit. Discrepancies generate a clear warning and require an explanatory note, but never block Month Close.
- **Month Close & Locking**: Explicit snapshot of month-to-date totals; locks the month from regular edits. Reopening requires Admin authorization with an audit reason.
...
- **Warn, Don't Block**: Real stalls have coin discrepancies, delayed TnG settlements, and unexpected expenses. Reconciliation flags mismatches clearly but never prevents recording or Month Close.
- **Lock Means Locked**: A closed month gives the family certainty that historical numbers are safe and stable, matching the finality of putting away last month's paper book.
